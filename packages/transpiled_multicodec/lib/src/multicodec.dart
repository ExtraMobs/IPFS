...
// code_table.go). All 603 entries extracted directly from that file's
// `Name Code = 0xNN // canonical-name` declarations, so codes stay
// byte-for-byte identical to the upstream registry (the previous ~30-entry
// table here had drifted: e.g. it invented `ipld-ns`/`ipfs-ns`/`ipns-ns` at
// codes 0x300/0x301/0x302, which the real table assigns to
// `ipns-record`/`libp2p-peer-record`/`libp2p-relay-rsvp` instead).
//
