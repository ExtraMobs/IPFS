...
//
// TestResultsFromIteratorUsingChan is not ported -- it exercises Go's
// channel-based `Results.Next()`, which this port deliberately doesn't
// have (see query.dart's header comment); TestResultsFromIteratorUsingRest
// already covers the same close-once contract via a different Results
// method.
...
void main() {
  test('TestNaiveQueryApply', () {
    ...
    run(const Query(filters: [FilterKeyCompare(FilterOp.equal, '/ab')]), sampleKeys, const [
      '/ab',
    ]);

    run(const Query(prefix: '/ab'), sampleKeys, const ['/ab/c', '/ab/cd', '/ab/ef', '/ab/fg']);
  ...
  test('TestOffset', () {
    void run(int offset, List<String> keys, List<String> expected) {
      final entries = [for (final k in keys) Entry(key: k)];
      final res = naiveOffset(resultsWithEntries(const Query(), entries), offset);
      testResults(res, expected);
    }

    run(0, sampleKeys, sampleKeys);
    run(10, sampleKeys, const []); // larger than input.
    run(2, sampleKeys, const ['/ab/ef', '/ab/fg', '/a', '/abce', '/abcf', '/ab']);
  });

  test('TestResultsFromIterator: close is called exactly once via nextSync', () {
    var i = 0;
    var closeCalled = 0;
    final results = resultsFromIterator(
      ...
      QueryIterator(
        next: () {
          if (i >= sampleKeys.length) return (const QueryResult(Entry(key: '')), false);
  ...
  test('TestStringer', () {
    ...
    q = const Query(offset: 10, limit: 10, orders: [OrderByValue(), OrderByKey()]);
    expect(q.toString(), equals('SELECT keys,vals ORDER [VALUE, KEY] OFFSET 10 LIMIT 10'));
