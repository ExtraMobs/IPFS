...
/// Parses `export 'src/<path>' show Name1, Name2;` statements (possibly
/// wrapped across lines) out of the barrel file, returning a map from each
/// exported name to the `lib/src/<path>` it came from.
Map<String, String> _parseBarrelExports(File barrelFile) {
  if (!barrelFile.existsSync()) return const {};
  ...
  for (final match in pattern.allMatches(content)) {
    ...
    final srcPath = 'lib/$relPath';
    for (final name in match.group(2)!.split(',')) {
      final trimmed = name.trim();
      if (trimmed.isNotEmpty) result[trimmed] = srcPath;
    }
  }
  return result;
}
...
String? _resolveTarget(String target, String fileDir) {
  if (target.startsWith('package:dart_ipfs_core/src/')) {
    return 'lib/src/${target.substring('package:dart_ipfs_core/src/'.length)}';
  }
...
void main() {
  ...
  // dart_ipfs_core's own test suite imports the public barrel
  // (package:dart_ipfs_core/dart_ipfs_core.dart) almost exclusively rather
  // than individual src/ files directly, unlike the umbrella dart_ipfs
  // package's tests. So "directly tested" is inferred by name: parse the
  // barrel's own `export 'src/...' show Name1, Name2;` statements into a
  // name -> src path map, then for each test file that imports the barrel,
  // mark a src file tested if any of its exported names appears in that
  // test's source.
  final barrelExports = _parseBarrelExports(File('lib/dart_ipfs_core.dart'));
  ...
  for (final file in testFiles) {
    ...
    for (final match in importPattern.allMatches(content)) {
      ...
      if (raw == 'package:dart_ipfs_core/dart_ipfs_core.dart') {
        importsBarrel = true;
        continue;
      }
      final target = _resolveTarget(raw, fileDir);
      if (target != null && target.startsWith('lib/src/')) {
        directlyTested.add(target);
      }
    }
    if (importsBarrel) {
      barrelExports.forEach((name, srcPath) {
        if (RegExp(r'\b' + RegExp.escape(name) + r'\b').hasMatch(content)) {
          directlyTested.add(srcPath);
        }
      });
    }
  }
  ...
  for (final mod in modules) {
    ...
    buf.writeln('---\n');
    buf.writeln('# Module `$mod` (`lib/src/$mod/`)\n');
    buf.writeln(
      '_Generated by `tool/generate_module_index.dart` via unresolved AST '
      '(`package:analyzer`). "Calls"/"referenced by" match by identifier '
      'name, not resolved type -- see the caveat at the top of the script. '
    ...
    for (final f in files) {
      ...
      for (final t in audit.types) {
        ...
        if (t.members.isEmpty) {
          buf.writeln('_(no public members)_\n');
        } else {
          for (final m in t.members) {
  ...
  for (final file in testFiles) {
    final group = _testGroupOf(file.path) ?? '(root)';
    final content = file.readAsStringSync();
  ...
  overview.writeln('# Architecture Index — Overview\n');
  overview.writeln(
    'Generated by `tool/generate_module_index.dart` on ${DateTime.now().toIso8601String()}. '
    'One file per module under [lib/](lib/) and [test/](test/), mirroring the repository\'s real structure. '
    'Do not edit these files by hand -- regenerate with `dart run tool/generate_module_index.dart` from the repo root. '
    'See [AGENTS.md](AGENTS.md) for how to use this index.\n',
  );
  overview.writeln('## Modules (`lib/src/*`)\n');
  overview.writeln('| Module | Files | Depends on |');
  overview.writeln('|---|---|---|');
  ...
  for (final g in (testGroups.keys.toList()..sort())) {
    overview.writeln('- [test/$g](test/${g.replaceAll('/', '_')}.md) (${testGroups[g]!.length} files)');
  }
  File('doc/architecture/README.md').writeAsStringSync(overview.toString());

  stdout.writeln(
    'Wrote: doc/architecture/README.md + ${modules.where((m) => (fileCount[m] ?? 0) > 0).length} '
    'lib/ modules + ${testGroups.length} test/ groups',
  );
}
