...
module: transport
kind: lib/src audit
generated: 2026-09-02T08:21:38.643318
---
...
Depends on: [core](core.md), [platform](platform.md), [proto](proto.md), [transpiled_base58[pkg]](), [transpiled_ipfs[pkg]](), [transpiled_libp2p[pkg]](), [transpiled_multiaddr[pkg]](), [transpiled_multiaddr_dns[pkg]](), [utils](utils.md)
...
- **connect** (method) — Connects to a peer using a circuit relay.
  - calls: debug, connectMultiaddr, add, CircuitRelayConnectionEvent, error, toString
...
- **toString** (method)
  - referenced by (by name) (name shared by 60 declarations -- not resolved to this one specifically, see caveat):
    ...
    - `lib/src/core/data_structures/memory_block_store.dart` (InMemoryBlockStore.getBlock)
    - `lib/src/core/data_structures/memory_block_store.dart` (InMemoryBlockStore.putBlock)
    - `lib/src/core/data_structures/memory_block_store.dart` (InMemoryBlockStore.removeBlock)
    ...
    - `lib/src/transport/quic/quic_transport.dart` (QuicTransport.dial)
    - `lib/src/transport/quic/quic_transport.dart` (QuicTransport.listen)
...
### top-level `createDnsResolver` (function)

- **createDnsResolver** (function) — The native (`dart:io`) DNS resolver, backed by real UDP sockets.
  - calls: SystemResolver
...
- **createDnsResolver** (function) — Browsers expose no raw-socket API, so there is no DNS resolver on web
...
- **a** (static field) — A record (IPv4 address).
  - referenced by (by name):
    - `lib/src/transport/dns/dns_message.dart` (decodeDnsMessage)
- **aaaa** (static field) — AAAA record (IPv6 address).
  - referenced by (by name):
    - `lib/src/transport/dns/dns_message.dart` (decodeDnsMessage)
- **txt** (static field) — TXT record.
  - referenced by (by name):
    - `lib/src/network/mdns_client.dart` (ResourceRecordQuery.text)
    - `lib/src/transport/dns/dns_message.dart` (decodeDnsMessage)
...
- **type** (field) — The record type ([DnsRecordType.a]/[aaaa]/[txt]).
  - referenced by (by name) (name shared by 38 declarations -- not resolved to this one specifically, see caveat):
    ...
    - `lib/src/transport/webrtc/webrtc_transport.dart` (WebRTCTransport.dial)
- **data** (field) — For A/AAAA: the address as text (dotted-decimal or colon-form). For
  - referenced by (by name) (name shared by 38 declarations -- not resolved to this one specifically, see caveat):
...
- **id** (field) — The transaction id, echoed from the query.
...
- **rcode** (field) — RFC 1035 §4.1.1 RCODE (0 = no error).
  - referenced by (by name):
    - `lib/src/transport/dns/udp_dns_client.dart` (UdpDnsClient.lookup)
- **records** (field) — The parsed answer-section records this client understands.
  - referenced by (by name):
    - `lib/src/transport/dns/udp_dns_client.dart` (UdpDnsClient.lookup)

### top-level `encodeDnsQuery` (function)

- **encodeDnsQuery** (function) — Encodes a standard recursive query for [name] and [type] (see
  - calls: BytesBuilder, _writeUint16, add, _encodeName, toBytes

### top-level `decodeDnsMessage` (function)

- **decodeDnsMessage** (function) — Parses a raw DNS message from the wire. Throws [FormatException] on any
  - calls: length, FormatException, _readUint16, _readName, a, add, DnsRecord, aaaa, _formatIp6, txt, _readTxtStrings, DnsResponse
...
- **lookupIPAddr** (method)
  - calls: lookup, address
- **lookupTXT** (method)
  - calls: lookup, txt, data
...
- **resolvers** (field) — Resolver IPv4/IPv6 addresses to query, in order.
- **timeout** (field) — Per-attempt response wait before retrying or moving to the next
  - referenced by (by name):
...
- **attemptsPerResolver** (field) — Retries per resolver before moving to the next one.
- **lookup** (method) — Queries every configured resolver in turn (retrying each up to
  - calls: _query, rcode, toList, where, records, type, TimeoutException
  - referenced by (by name) (name shared by 4 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/network/mdns_client_io.dart` (MDnsClientIO.lookup)
    - `lib/src/services/gateway/content_type_handler.dart` (ContentTypeHandler.detectContentType)
...
### top-level `defaultDnsResolvers` (variable)

- **defaultDnsResolvers** (variable) — Default fallback resolvers, tried in order.
...
- **close** (method) — Releases any resources held by this client (e.g. an internally
...
- **isInitialized** (method)
  - referenced by (by name) (name shared by 3 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/core/ipfs_node/ipfs_node.dart` (IPFSNode.hasActiveDhtClient)
...
- **initialize** (method)
  - calls: warning, debug, _generateKeyPairFromSeed, _generateKeyPair, enableQuic, network, _probeQuicTransport, error, StateError
  - referenced by (by name) (name shared by 8 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/core/ipfs_node/ipfs_web_node.dart` (IPFSWebNode.start)
...
- **start** (method)
  - calls: warning, initialize, debug, _loadPrivateNetworkPsk, info, _buildListenAddresses, ResourceManagerImpl, FixedLimiter, WebRTCTransport, network, WebRTCDirectTransport, WebTransportTransport, TCPTransport, PnetTransportWrapper, transport, Libp2p, enableQuic, add, enableWebTransport, enableWebRtc, new_, listenAddrs, identity, userAgent, security, DartIpfsNoiseSecurity, host, register, start, notify, NotifyBundle, toString, remotePeer, remoteMultiaddr, ConnectionEvent, connected, remove, disconnected, first, id, error, StateError
  - referenced by (by name) (name shared by 66 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/core/ipfs_node/auto_nat_handler.dart` (AutoNATHandler.start)
    - `lib/src/core/ipfs_node/content_routing_handler.dart` (ContentRoutingHandler.start)
    - `lib/src/core/ipfs_node/ipfs_node.dart` (IPFSNode.fromBuildCfg)
    ...
    - `lib/src/transport/circuit_relay_client_io.dart` (CircuitRelayClient.start)
- **stop** (method)
  - calls: warning, debug, close, clear, values, isClosed, info, error
  - referenced by (by name) (name shared by 59 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/core/ipfs_node/auto_nat_handler.dart` (AutoNATHandler.stop)
    - `lib/src/core/ipfs_node/content_routing_handler.dart` (ContentRoutingHandler.stop)
    - `lib/src/core/ipfs_node/ipfs_node.dart` (IPFSNode.close)
    ...
    - `lib/src/routing/content_routing.dart` (ContentRouting.stop)
- **connect** (method)
  - calls: _checkStarted, addrs, toString, id, fromString, PeerId, toBase58, MultiAddr, toAddrString, connect, AddrInfo, isNotEmpty, setAddrs, addrBook, peerStore, Duration, StateError, add, error
  - referenced by (by name) (name shared by 4 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/transport/libp2p_router.dart` (Libp2pRouter.connectMultiaddr)
    - `lib/src/transport/webrtc/webrtc_transport.dart` (WebRTCTransport.dial)
- **connectMultiaddr** (method)
  - calls: _checkStarted, debug, _extractPeerIdFromMultiaddr, ArgumentError, split, fromString, PeerId, MultiAddr, addAddrs, addrBook, peerStore, Duration, connect, AddrInfo, fromBase58, parse, Multiaddr, StateError, add, error
  - referenced by (by name) (name shared by 2 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/core/ipfs_node/ipfs_web_node.dart` (IPFSWebNode.start)
...
- **sendMessage** (method)
  - calls: _checkStarted, verbose, Context, Duration, fromString, PeerId, newStream, _encodeLengthPrefix, length, write, fromList, close, error, NetworkException, cancel
  - referenced by (by name) (name shared by 5 declarations -- not resolved to this one specifically, see caveat):
    ...
    - `lib/src/transport/webrtc/webrtc_transport.dart` (WebRTCTransport.dial)
- **sendRequest** (method)
  - calls: _checkStarted, verbose, Context, Duration, fromString, PeerId, newStream, _encodeLengthPrefix, length, write, fromList, _readLengthPrefixedMessage, close, error, cancel
  - referenced by (by name) (name shared by 5 declarations -- not resolved to this one specifically, see caveat):
...
- **sendMessageWithResponse** (method)
  - calls: _checkStarted, Duration, verbose, Context, fromString, PeerId, newStream, _encodeLengthPrefix, length, write, fromList, _readLengthPrefixedMessage, TimeoutException, close, error, cancel
  - referenced by (by name) (name shared by 2 declarations -- not resolved to this one specifically, see caveat):
...
- **message** (field) — The error message.
  - referenced by (by name) (name shared by 43 declarations -- not resolved to this one specifically, see caveat):
    ...
    - `lib/src/protocols/pubsub/gossipsub/message_cache.dart` (MessageCache.getForIWant)
    - `lib/src/utils/keystore.dart` (Keystore.verifySignature)
- **cause** (field) — The underlying error, if this wraps one.
- **toString** (method)
  - referenced by (by name) (name shared by 60 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/core/data_structures/blockstore.dart` (BlockStore.has)
    - `lib/src/core/data_structures/blockstore.dart` (BlockStore.get)
...
- **localIdentityKey** (field) — This node's libp2p identity private key, used to sign the Noise
- **protocolId** (method)
...
### top-level `noiseProtocolId` (variable)

- **noiseProtocolId** (variable) — The multistream-select protocol ID for Noise, per go-libp2p (and
...
- **isClosed** (method)
  ...
  - referenced by (by name) (name shared by 15 declarations -- not resolved to this one specifically, see caveat):
    ...
    - `lib/src/transport/quic/quic_listener.dart` (QuicListener.close)
    - `lib/src/transport/quic/quic_p2p_stream.dart` (QuicP2PStream.read)
    - `lib/src/transport/quic/quic_p2p_stream.dart` (QuicP2PStream.close)
    - `lib/src/transport/quic/quic_p2p_stream.dart` (QuicP2PStream.closeRead)
    - `lib/src/transport/webrtc/signaling_protocol.dart` (SignalingProtocol.handleStream)
...
- **write** (method)
  ...
  - referenced by (by name) (name shared by 10 declarations -- not resolved to this one specifically, see caveat):
    ...
    - `lib/src/transport/libp2p_router.dart` (Libp2pRouter.registerProtocolHandler)
    - `lib/src/transport/pnet/pnet_transport_conn.dart` (PnetTransportConn.create)
    - `lib/src/transport/quic/quic_p2p_stream.dart` (QuicP2PStream.write)
...
- **state** (method)
  ...
  - referenced by (by name) (name shared by 9 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/protocols/dht/query_peerset.dart` (QueryPeerset.setState)
    - `lib/src/protocols/dht/query_peerset.dart` (QueryPeerset.getState)
...
## `lib/src/transport/quic/libp2p_tls_extension.dart`
...
- **valid** (field) — Whether the certificate is valid and the peer identity was established.
- **peerId** (field) — The libp2p [PeerId] derived from the certificate's embedded public key.
...
- **publicKey** (field) — The peer's public key extracted from the libp2p TLS extension.
...
- **failureReason** (field) — The reason verification failed, when [valid] is `false`.
- **expectedPeerId** (field) — The expected peer ID when verification failed due to a mismatch.
- **failureDetail** (field) — A human-readable description of the failure, when [valid] is `false`.
- **toString** (method)
  - referenced by (by name) (name shared by 60 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/core/data_structures/blockstore.dart` (BlockStore.has)
    - `lib/src/core/data_structures/blockstore.dart` (BlockStore.get)
...
- **verify** (method) — Verifies [certBytes] (raw DER-encoded X.509) and derives the peer
  - calls: parseX509, failed, parseError, parseLibp2pExtension, noExtension, oid, Libp2pExtension, signedKey, fromList, data, publicKey, type, ed25519, Libp2pKeyType, length, invalidSignature, fromRawBytes, Ed25519PublicKey, rsa, secp256k1, ecdsa, unsupportedKeyType, subjectPublicKeyInfo, encode, signature, verify, fromPublicKey, PeerId, peerIdMismatch, ok
  - referenced by (by name) (name shared by 7 declarations -- not resolved to this one specifically, see caveat):
...
- **expectedPeerId** (field) — The expected peer ID (e.g. from the dialed multiaddr).
- **actualPeerId** (field) — The peer ID derived from the peer's certificate.
- **detail** (field) — A human-readable description of the mismatch.
- **toString** (method)
  - referenced by (by name) (name shared by 60 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/core/data_structures/blockstore.dart` (BlockStore.has)
    - `lib/src/core/data_structures/blockstore.dart` (BlockStore.get)
...
- **reason** (field) — The reason verification failed.
  - referenced by (by name) (name shared by 17 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/core/plugins/plugin_audit_log.dart` (PluginAuditLog.recordException)
- **detail** (field) — A human-readable description of the failure.
- **toString** (method)
  - referenced by (by name) (name shared by 60 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/core/data_structures/blockstore.dart` (BlockStore.has)
    - `lib/src/core/data_structures/blockstore.dart` (BlockStore.get)
...
### top-level `libp2pTlsHandshakePrefix` (variable)

- **libp2pTlsHandshakePrefix** (variable) — The prefix used in the libp2p TLS handshake signed message.

## `lib/src/transport/quic/quic.dart`

Internal barrel combining this package's QUIC transport adapter

_Directly tested._

## `lib/src/transport/quic/quic_listener.dart`
...
- **isClosed** (method)
  - referenced by (by name) (name shared by 15 declarations -- not resolved to this one specifically, see caveat):
    ...
    - `lib/src/transport/webtransport/webtransport_session.dart` (WebTransportSessionManager.cleanupInactive)
- **accept** (method)
  - calls: isNotEmpty, removeAt, QuicConnection, first
  - referenced by (by name) (name shared by 4 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/transport/pnet/pnet_listener.dart` (PnetListener.accept)
- **close** (method)
  - calls: cancel, isClosed, close
  - referenced by (by name) (name shared by 33 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/core/events/event_bus.dart` (EventBus.dispose)
    - `lib/src/core/ipfs_node/datastore_handler.dart` (DatastoreHandler.stop)
    - `lib/src/core/ipfs_node/datastore_handler.dart` (DatastoreHandler.exportCAR)
...
## `lib/src/transport/quic/quic_p2p_stream.dart`
...
- **getQuicStream** (method) — Returns the QUIC stream with the given [id], or `null` if it does not exist.
  - referenced by (by name) (name shared by 2 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/transport/quic/quic_transport.dart` (QuicConnection.getQuicStream)
- **isEstablished** (method) — Whether the QUIC handshake is complete.
  - referenced by (by name) (name shared by 2 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/transport/quic/quic_transport.dart` (QuicConnection.isEstablished)
- **openBidirectionalStream** (method) — Opens a new bidirectional stream and returns its stream ID.
  - referenced by (by name) (name shared by 4 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/transport/quic/quic_transport.dart` (QuicConnection.openBidirectionalStream)
    - `lib/src/transport/quic/quic_transport.dart` (QuicConnection.newStream)
    - `lib/src/transport/webtransport/webtransport_session.dart` (WebTransportSession.openBidirectionalStream)
...
- **read** (method)
  - calls: StateError, delayed, Duration, length, Uint8List, add, setRange, sublistView, clear, addAll, Completer, _ReadRequest, listen, incomingData, isClosed, fromList, _drainReadBuffer, future
  - referenced by (by name) (name shared by 7 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/network/mdns_client_io.dart` (MDnsClientIO.startServer)
    - `lib/src/protocols/bitswap/message.dart` (Message.fromNetStream)
    - `lib/src/services/rpc/mfs_handlers.dart` (MFSHandlers.handleFilesRead)
    ...
    - `lib/src/transport/webtransport/webtransport_dialer_web.dart` (WebTransportStreamWeb.read)
- **write** (method)
  - calls: StateError, write, fromList
  - referenced by (by name) (name shared by 10 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/core/ipfs_node/datastore_handler.dart` (DatastoreHandler.exportCAR)
    - `lib/src/platform/platform_io.dart` (IpfsPlatformIO.promptPassword)
    - `lib/src/services/rpc/mfs_handlers.dart` (MFSHandlers.handleFilesWrite)
    ...
    - `lib/src/utils/encoding.dart` (EncodingUtils.base32LowerEncode)
- **incoming** (method)
- **stream** (method) — A Dart [Stream] of incoming data for this QUIC stream.
  - calls: stream
  - referenced by (by name) (name shared by 2 declarations -- not resolved to this one specifically, see caveat):
    ...
    - `lib/src/transport/quic/quic_listener.dart` (QuicListener.connectionStream)
...
- **close** (method)
  - calls: closeWrite, closeRead, cancel, isClosed, close
  - referenced by (by name) (name shared by 33 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/core/events/event_bus.dart` (EventBus.dispose)
    - `lib/src/core/ipfs_node/datastore_handler.dart` (DatastoreHandler.stop)
    - `lib/src/core/ipfs_node/datastore_handler.dart` (DatastoreHandler.exportCAR)
...
## `lib/src/transport/quic/quic_transport.dart`
...
- **dial** (method)
  - calls: StateError, fine, parse, Multiaddr, toString, dial, QuicConnection, _toIpfsMultiaddr
  - referenced by (by name) (name shared by 8 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/transport/pnet/pnet_transport_wrapper.dart` (PnetTransportWrapper.dial)
    - `lib/src/transport/webtransport/webtransport_transport.dart` (WebTransportTransport.dial)
- **listen** (method)
  - calls: StateError, fine, parse, Multiaddr, toString, listen, QuicListener
  - referenced by (by name) (name shared by 5 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/core/ipfs_node/pubsub_handler.dart` (PubSubHandler.start)
    - `lib/src/network/mdns_client_io.dart` (MDnsClientIO.startServer)
    - `lib/src/platform/platform_web.dart` (IpfsPlatformWeb.exists)
...
- **isClosed** (method)
  - referenced by (by name) (name shared by 15 declarations -- not resolved to this one specifically, see caveat):
    ...
    - `lib/src/transport/webtransport/webtransport_listener.dart` (WebTransportListener.isClosed)
    - `lib/src/transport/webtransport/webtransport_session.dart` (WebTransportSessionManager.cleanupInactive)
- **quicConnection** (method) — The underlying [quic_lib] connection object.
  ...
  - referenced by (by name):
    ...
    - `lib/src/transport/quic/quic_transport.dart` (QuicConnection.newStream)
    - `lib/src/transport/quic/quic_transport.dart` (QuicConnection.streams)
- **getQuicStream** (method)
  - calls: quicConnection, getStream, streamManager, getQuicStream
- **isEstablished** (method)
  - calls: quicConnection, isEstablished
- **openBidirectionalStream** (method)
  - calls: quicConnection, openBidirectionalStream, StateError
  - referenced by (by name) (name shared by 4 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/transport/quic/quic_transport.dart` (QuicConnection.newStream)
    - `lib/src/transport/webtransport/webtransport_session.dart` (WebTransportSession.openBidirectionalStream)
- **localPeer** (method)
...
- **remotePeer** (method) — The remote peer identity for this QUIC connection.
  - calls: peerId, fromString, PeerId, toBase58, StateError
  - referenced by (by name) (name shared by 4 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/transport/libp2p_router.dart` (Libp2pRouter.start)
...
- **verifyPeer** (method) — Validates that the peer's selected ALPN is acceptable and a [PeerId] has
  - calls: validateAlpn, peerId
- **verifyPeerCertificate** (method) — Verifies the peer's certificate using the libp2p TLS extension.
  - calls: DefaultCryptoBackend, verifyPeerCertificate
- **verifyPeerFromHandshake** (method) — Verifies the peer using the certificate captured during the QUIC
  - calls: DefaultCryptoBackend, verifyPeerCertificateFromHandshake
- **localMultiaddr** (method)
  - referenced by (by name) (name shared by 4 declarations -- not resolved to this one specifically, see caveat):
...
- **stat** (method)
  - calls: _QuicConnStats, Stats, inbound, Direction, outbound, now
  - referenced by (by name) (name shared by 8 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/services/rpc/mfs_handlers.dart` (MFSHandlers.handleFilesLs)
    - `lib/src/services/rpc/mfs_handlers.dart` (MFSHandlers.handleFilesStat)
    - `lib/src/transport/pnet/pnet_transport_conn.dart` (PnetTransportConn.stat)
- **scope** (method)
  ...
  - referenced by (by name) (name shared by 7 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/transport/pnet/pnet_transport_conn.dart` (PnetTransportConn.scope)
- **newStream** (method)
  - calls: quicConnection, StateError, delayed, Duration, openBidirectionalStream, QuicP2PStream, outbound, Direction
  - referenced by (by name) (name shared by 4 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/transport/libp2p_router.dart` (Libp2pRouter.sendMessage)
    - `lib/src/transport/libp2p_router.dart` (Libp2pRouter.sendRequest)
    - `lib/src/transport/libp2p_router.dart` (Libp2pRouter.sendMessageWithResponse)
    ...
    - `lib/src/transport/webrtc/webrtc_transport.dart` (WebRTCTransport.dial)
- **streams** (method)
  - calls: quicConnection, streams, streamManager, add, QuicP2PStream, streamId, inbound, Direction
  - referenced by (by name) (name shared by 4 declarations -- not resolved to this one specifically, see caveat):
...
- **socket** (method)
  - calls: UnsupportedError
  - referenced by (by name) (name shared by 2 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/transport/pnet/pnet_transport_conn.dart` (PnetTransportConn.socket)
- **setReadTimeout** (method)
  - referenced by (by name) (name shared by 2 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/transport/pnet/pnet_transport_conn.dart` (PnetTransportConn.setReadTimeout)
- **setWriteTimeout** (method)
  - referenced by (by name) (name shared by 2 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/transport/pnet/pnet_transport_conn.dart` (PnetTransportConn.setWriteTimeout)
- **notifyActivity** (method)
  - referenced by (by name) (name shared by 2 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/transport/pnet/pnet_transport_conn.dart` (PnetTransportConn.notifyActivity)
...
- **stop** (method) — Stops the router and disconnects all peers.
  - referenced by (by name) (name shared by 59 declarations -- not resolved to this one specifically, see caveat):
    ...
    - `lib/src/protocols/pubsub/gossipsub/gossipsub_pubsub_adapter.dart` (GossipsubPubSubAdapter.stop)
    - `lib/src/routing/content_routing.dart` (ContentRouting.stop)
- **connect** (method) — Connects to a peer using its ID and known transport addresses.
  - referenced by (by name) (name shared by 4 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/transport/libp2p_router.dart` (Libp2pRouter.connect)
    - `lib/src/transport/libp2p_router.dart` (Libp2pRouter.connectMultiaddr)
    - `lib/src/transport/webrtc/webrtc_transport.dart` (WebRTCTransport.dial)
- **connectMultiaddr** (method) — Compatibility entry point for callers that have a complete p2p address.
  - referenced by (by name) (name shared by 2 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/core/ipfs_node/ipfs_web_node.dart` (IPFSWebNode.start)
    - `lib/src/core/ipfs_node/network_handler_io.dart` (NetworkHandler.connectToPeer)
    - `lib/src/core/ipfs_node/network_handler_io.dart` (NetworkHandler.canConnectDirectly)
...
- **setDHTRoutingTable** (method) — Sets the DHT routing table, making it available via [dhtRoutingTable].
  - referenced by (by name) (name shared by 2 declarations -- not resolved to this one specifically, see caveat):
    - `lib/src/protocols/dht/dht_client.dart` (DHTClient.initialize)
...
- **hashCode** (method)
  ...
  - referenced by (by name) (name shared by 26 declarations -- not resolved to this one specifically, see caveat):
    ...
    - `lib/src/utils/immutable_bytes.dart` (ImmutableBytes.hashCode)
    - `lib/src/utils/typed_map.dart` (TypedMap.hashCode)
...
- **stream** (method) — Gets the stream.
  ...
  - referenced by (by name) (name shared by 2 declarations -- not resolved to this one specifically, see caveat):
    ...
    - `lib/src/transport/libp2p_router.dart` (Libp2pRouter.receiveMessages)
    - `lib/src/transport/quic/quic_listener.dart` (QuicListener.connectionStream)
    - `lib/src/transport/quic/quic_p2p_stream.dart` (QuicP2PStream.stream)
    - `lib/src/transport/webrtc/peer_connection_web.dart` (PeerConnectionWeb.onIceCandidate)
    - `lib/src/transport/webrtc/peer_connection_web.dart` (PeerConnectionWeb.onDataChannel)
    - `lib/src/transport/webrtc/signaling_protocol.dart` (SignalingProtocol.messages)
    - `lib/src/transport/webrtc/webrtc_transport.dart` (WebRTCListener.connectionStream)
