...
| `core/peer` | `packages/transpiled_libp2p/lib/src/core/peer/peer_id.dart` (`ID` -- `peer.go`) + `packages/transpiled_libp2p/lib/src/core/peer/addr_info.dart` (`AddrInfo` -- `addrinfo.go`) | **portado com paridade comprovada** (exceto `PeerRecord`, ver nota) | `IDFromPublicKey`/`IDFromPrivateKey` corrigidos e completos (`PeerId.fromPubKey`/`fromPrivateKey`, ver nota acima sobre o bug original). Resto de `peer.go` completo: `IDFromBytes`, `Decode` (base58 legado ou CID), `FromCid`/`ToCid` (`libp2p-key` codec via `transpiled_cid`), `ID.Validate`, `ID.ShortString`, `MatchesPublicKey`/`MatchesPrivateKey`, `ExtractPublicKey`, e `IDSlice`'s ordenação bytewise (`PeerId implements Comparable<PeerId>`, já que `sort.Interface` não existe em Dart). `peer_serde.go` completo: `MarshalBinary`/`UnmarshalBinary`, `MarshalText`/`UnmarshalText` (o par `Marshal`/`MarshalTo`/`Size` do gogo-proto não foi portado -- é idêntico a `MarshalBinary`, não é uma lacuna real). `addrinfo.go`/`addrinfo_serde.go` completos: `SplitAddr` (precisou `Multiaddr.splitLast()`, adicionado ao `transpiled_multiaddr`), `IDFromP2PAddr`, `AddrInfoFromString`/`AddrInfoFromP2pAddr`/`AddrInfoToP2pAddrs`, `AddrInfosFromP2pAddrs`/`AddrInfosToIDs`, `Loggable`, serialização JSON (`toJson`/`fromJson`). Note "`Set`" citada numa versão anterior desta linha não existe na versão vendorizada de `go-libp2p` em `go-ipfs-reference/` -- foi removida do pacote real em algum momento; correção do registro, não uma lacuna. Testes com vetores reais do próprio `peer_test.go`/`addrinfo_test.go` do go-libp2p (a chave RSA e o peer ID `QmcJeseo...` do keyset `man`, os multiaddrs de `addrinfo_test.go`) em `test/peer_id_test.dart`/`test/addr_info_test.dart`. **Não portado**: `record.go` (`PeerRecord`, `NewPeerRecord`, `Sign`/`ConsumeEnvelope` via `Envelope`) -- depende de `core/record` (o sistema de `Envelope`/`Record` assinado), que ainda não foi portado; retomar `PeerRecord` junto de `core/record`, não faz sentido portar um sem o outro. |
| `core/peer/pb` |  | não iniciado | Só usado por `record.go`'s `PeerRecord` (mensagem `PeerRecord{PeerId, Addresses, Seq}`) -- adiado junto com `core/record`/`PeerRecord`, ver nota da linha `core/peer`. |
