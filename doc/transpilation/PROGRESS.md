...
| `linking/cid` | `packages/transpiled_ipld_prime/lib/src/linking/cid/cid_link.dart` (+ `lib/linking_cid.dart`) | **portado com paridade comprovada** | `Link`/`LinkPrototype` baseados em `CID`, equivalentes a `cidLink.Link`/`LinkPrototype`; preserva string, bytes binários, prefixo e construção por digest. Teste Dart valida round-trip com CID real; integração com registry/linking system completo continua pendente. |
