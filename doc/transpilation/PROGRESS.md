...
| Pacote Go | Destino em `lib/src/` | Status | Notas |
|---|---|---|---|
| `(root)` | `packages/dart_ipfs_core/lib/src/multiaddr/dns_resolver.dart` | portado com paridade comprovada (orquestração; sem I/O real ainda) | **Este é o pacote que disparou toda a transpilação** (nó ficava em 0 peers por causa da resolução de `/dnsaddr/`). Portado: a interface `BasicResolver` (`lookupIPAddr`/`lookupTXT`), `MockResolver` (mock.go, usado nos testes de paridade), a classe `Resolver` inteira (resolve.go: `resolve()`, resolvers por domínio/TLD com match left-to-right via `withDomainResolver`/`withDefaultResolver`, o algoritmo de sufixo do dnsaddr com contagem de componentes), `dnsMatches` (util.go `Matches`), `isFqdn`/`fqdn` (com um bug real de paridade de bits encontrado e corrigido -- minha primeira tentativa contava as barras invertidas à direita mas invertia a paridade esperada por um erro de off-by-one na definição do índice, then corrigido conferindo contra a tabela de casos de `TestIsFqdn`/`TestFqdn` do próprio go-multiaddr-dns). Precisou adicionar `splitFunc`/`splitFirst`/`forEach` no `Multiaddr` do `go-multiaddr` (útil de qualquer forma, e exatamente os util.go que tinham ficado de fora na primeira passada). 30 testes de paridade em `test/dns_resolver_parity_test.dart`, usando os fixtures reais de `resolve_test.go` (incluindo o helper `resolveAllDNS` que o próprio teste Go define, pra resolver todos os componentes DNS recursivamente) -- tudo via `MockResolver`, sem rede real. **Deliberadamente não portado ainda, por instrução explícita do usuário ("pra lib da stdlib que precisarmos implementar vamos fazer como uma etapa separada"):** um `BasicResolver` concreto de verdade. `dart:io`'s `InternetAddress.lookup()` cobriria `lookupIPAddr` (dns/dns4/dns6) hoje sem lacuna nenhuma, mas `lookupTXT` (necessário pra `/dnsaddr/`) não tem equivalente em `dart:io` -- precisa de um cliente DNS de verdade (UDP bruto conforme RFC 1035, opção B já escolhida pelo usuário antes do pivô, ou avaliar `dnslib` do pub.dev primeiro pela metodologia). Esse resolver concreto é código de I/O de plataforma, então o lugar certo pra ele é a árvore do `dart_ipfs` (que já usa `dart:io` em `network_handler_io.dart`), não o `dart_ipfs_core` (que se mantém livre de `dart:io` de propósito, pra compatibilidade web). |
