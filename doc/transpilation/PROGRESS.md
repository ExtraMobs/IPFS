...
| `core/network` | `packages/transpiled_libp2p/lib/src/core/network/network.dart` | portado sem teste de paridade | Enums `Direction`/`Connectedness`/`Reachability`, `MessageSizeMax`, `Notifiee`/`NotifyBundle`/`NoopNotifiee`; interfaces completas `Network`/`Conn` e transporte ainda pendentes. |
