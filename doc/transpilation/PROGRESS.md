...
| `core/crypto` | `packages/dart_ipfs_core/lib/src/crypto/key_types.dart` (`Key`/`PrivKey`/`PubKey`/`KeyType`) + `rsa_key.dart` (`RsaPrivateKey`/`RsaPublicKey`) | em andamento (RSA portado com paridade comprovada; Secp256k1/ECDSA não iniciados) | RSA: `MinRsaKeyBits=2048`, PKCS1 DER privado (`x509.MarshalPKCS1PrivateKey`/`ParsePKCS1PrivateKey`) e PKIX DER público (`x509.MarshalPKIXPublicKey`/PKIX `SubjectPublicKeyInfo`) via ASN.1 do `package:pointycastle` (`ASN1Sequence`/`ASN1Integer`/`ASN1ObjectIdentifier`/`ASN1BitString`/`ASN1Null`), sign/verify SHA-256+PKCS1v1.5 via `pc.RSASigner` (equivalente a `rsa.SignPKCS1v15`/`VerifyPKCS1v15`). Geração via `pc.RSAKeyGenerator`+`FortunaRandom`. Validado byte-a-byte contra uma chave RSA-2048 real gerada por `crypto/rsa`+`crypto/x509` do Go (`go-ipfs-reference/noise_vectors/rsa_vectors.go`, auto-verificada em Go antes de virar fixture): DER privado/público re-codificam idênticos ao original, assinatura Dart sobre a mesma chave/mensagem bate byte a byte com a assinatura do Go (PKCS1v1.5 é determinístico), e `verify()` aceita a assinatura real do Go -- ver `packages/dart_ipfs_core/test/rsa_key_parity_test.dart`. Falta: Secp256k1 e ECDSA (`core/crypto/{secp256k1,ecdsa}_go.go`), e o gap real que impede conexão com peers RSA continua sendo o handshake Noise (`p2p/security/noise`, hardcoded pra Ed25519 -- ver nota acima), não este pacote; RSA aqui é pré-requisito pro payload de assinatura do Noise, não a correção em si. |
...
| `p2p/security/noise` | `lib/src/transport/noise/noise_state.dart` (só o núcleo do protocolo Noise -- `handshake.go`/`session.go`/`transport.go` equivalentes ainda não portados) | em andamento -- núcleo com paridade comprovada, wiring libp2p ainda não | Portado: `CipherState`/`SymmetricState`/`HandshakeState` completos pro cipher suite fixo que o go-libp2p sempre usa (`DH25519, CipherChaChaPoly, HashSHA256`, padrão `XX`) -- não é uma lib Noise genérica de propósito geral, não tem porquê ser (go-libp2p nunca usa outro pattern/suite). Usa `package:cryptography` (já dependência) pra X25519/ChaCha20-Poly1305/SHA256; o HKDF de 2-3 saídas do próprio spec do Noise (seção 4.3, distinto do RFC 5869) foi escrito à mão sobre HMAC-SHA256, já que `package:cryptography`'s `Hkdf` implementa RFC 5869 exatamente, não a variante do Noise. **Validado byte a byte contra um vetor real**: instalei `flynn/noise` (a lib Go que o `p2p/security/noise` do go-libp2p realmente usa) via `go get` em `go-ipfs-reference/noise_vectors/`, rodei o `vectorgen` que a própria lib inclui (gera vetores de teste com chaves fixas, formato Cacophony/noise-c), extraí a entrada `Noise_XX_25519_ChaChaPoly_SHA256` com prólogo+payloads (sem PSK), e comparei minha implementação Dart byte a byte contra as 3 mensagens de handshake E contra as 2 mensagens de transporte pós-handshake (prova que `Split()` deriva as chaves certas) -- bate exato. Também um teste de auto-interop (duas sessões Dart completando handshake uma com a outra) prova consistência ponta a ponta. 2 testes em `test/transport/noise/noise_state_test.dart`. Achei e corrigi dois erros reais nas MINHAS PRÓPRIAS asserções de teste ao comparar contra o vetor (não no protocolo em si): (1) assumi que as mensagens de transporte pós-handshake do vetor alternavam entre as partes -- na verdade o `vectorgen` usa `cs0`/`cs1` do MESMO lado (quem escreveu a última mensagem de handshake) pras duas, sem alternar; (2) no teste de auto-interop, emparelhei errado enc/dec dos dois lados (`aSplit.$1` com `bSplit.$2` em vez de `bSplit.$1` -- os dois lados derivam os MESMOS dois `CipherState`s do mesmo chaining key final, e é o papel initiator/responder que decide qual é enc e qual é dec, não quem calculou o `Split()`). **Falta pro handshake libp2p completo**: o payload assinado (`NoiseHandshakePayload{IdentityKey, IdentitySig, Extensions}`, protobuf de 3 campos, protocolo `noise-libp2p-static-key:` + chave estática Noise assinada com a chave de identidade libp2p) -- a assinatura/verificação real de peers com chave RSA depende do Tier 3 (`core/crypto`) ter suporte a RSA `Sign`/`Verify`, então esse fio só fecha de ponta a ponta depois dos dois tiers. Também falta: `session.go`/`transport.go` equivalentes (framing de 2 bytes de comprimento, leitura/escrita de stream pós-handshake), e a integração de fato substituindo o uso do `ipfs_libp2p` no transporte real. |
| `p2p/security/noise/pb` | (protobuf mínimo hand-rolled, planejado junto com o payload signing acima -- ainda não escrito) | não iniciado |  |
